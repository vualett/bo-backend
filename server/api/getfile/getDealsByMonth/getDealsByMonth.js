...
API.get('/get/dealsbymonth', limiter, async (req, res) => {
  ...
  try {
    ...
    const dataFormated = data.map((d) => ({
      date: d.date,
      count: d.cashAdvanceCount,
      TotalPrincipal: d.cashAdvancTotalPrincipal,
      TotalPrincipalPlusFee: d.cashAdvancTotalPrincipalPlusFee
    }));
    ...
    logger.error(`[API:/get/dealsbymonth]${error}`);
