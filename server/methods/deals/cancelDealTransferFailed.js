...
export default async function cancelDealIfTransferFailed(id) {
  check(id, String);
  ...
  if (transfer) {
    ...
    if (!updated) {
      ...
      insertDataChangesLog({
        ...
        documentID: deal._id,
        operation: 'update',
        method: 'cancelDealIfTransferFailed',
    ...
    Meteor.users.update(
      { _id: deal.userId },
      { $set: { currentCashAdvance: false }, $inc: { 'metrics.cashAdvances.count': -1 } }
    );
...
Meteor.methods({ 'deals.cancelIfTransferFailed': cancelDealIfTransferFailed });
